#!/usr/bin/env python3
"""
Thermal HIL Live UI (multi): subscribe to Zenoh topic, buffer CSV lines, and display
all columns in a live-updating GUI with max 6 columns per row (extra rows as needed).

Same as thermal_hil_live_ui.py but shows all CSV columns (no filtering) and lays out
the table with COLS_PER_ROW=6 per row.

Usage:
  python tests/thermal_hil_live_ui_multi.py
  python tests/thermal_hil_live_ui_multi.py --out-topic other/topic --endpoint tcp/192.168.1.10:1234
  python tests/thermal_hil_live_ui_multi.py --terminal
  python tests/thermal_hil_live_ui_multi.py --web
...
# All columns enabled (multi UI shows them in a grid with COLS_PER_ROW per row)
...
<head>
  ...
  <title>Thermal HIL Live Data (Multi)</title>
  <style>
...
def main() -> int:
    ...
    root = tk.Tk()
    root.title("Thermal HIL Live Data (Multi)")
    ...
    SPACER_ROWS = 2
    ROWS_PER_BLOCK = 2 + SPACER_ROWS  # header + data + 2 spacer
