...
<body>
  <div class="app">
    <header class="header">
      ...
      <div class="route-label">Sunnyvale → Tahoe → Sunnyvale (round trip) · __TOTAL_KM__ km · Loop</div>
      <div class="controls">
    ...
    function updateUI(data) {
      ...
      if (socWarning) {
        ...
        if (!data.charging && soc <= 20 && soc > 0) {
          ...
          socWarning.textContent = 'Battery empty — navigation stopped. Charging at Lake Tahoe in ' + data.empty_recovery_seconds_left + ' s.';
        } else if (!data.charging && soc <= 0) {
          socWarning.className = 'soc-warning empty';
          socWarning.textContent = 'Battery empty — navigation stopped';
        }
      }
      ...
      if (data.charging && chargingBlock && chargingFill && chargingText) {
        ...
        if (chargingLabel) chargingLabel.textContent = data.recovery_charging ? 'Charge at Lake Tahoe' : 'Charging at destination';
...
def main() -> int:
    ...
    charge_step_pct = 1  # charging at destination: +5% per tick
