#!/usr/bin/env python3
"""
EV Live Navigation: Sunnyvale → Death Valley National Park (real-world route).
Route from sunnyvale_deathvalley_route.json: real-world waypoints, chargers, weather, wind, elevation.
...
ROUTE_JSON = Path(__file__).resolve().parent.joinpath("csv_data", "sunnyvale_deathvalley_route.json").expanduser().resolve()
...
def _make_round_trip(route_data: dict) -> dict:
    """Append return leg (reverse of waypoints) so route goes back to origin. Mutates and returns route_data."""
...
def _web_html(route_data: dict) -> str:
    ...
    max_range_km = route_data.get("max_range_km", 300)
...
<head>
  ...
  <title>EV Live — Sunnyvale → Death Valley</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .soc-warning { margin: 10px 0; padding: 10px 12px; border-radius: 8px; font-size: 0.9rem; font-weight: 500; display: none; }
    .soc-warning.low { display: block; background: rgba(234, 179, 8, 0.2); border: 1px solid #eab308; color: #eab308; }
    .soc-warning.empty { display: block; background: rgba(244, 63, 94, 0.2); border: 1px solid var(--danger); color: var(--danger); }
    ...
    .destination-status { margin: 10px 0; padding: 12px 14px; border-radius: 10px; font-size: 0.95rem; font-weight: 600; display: none; }
    .destination-status.approaching { display: block; background: rgba(34, 195, 166, 0.2); border: 1px solid var(--accent); color: var(--accent); }
    .destination-status.reached { display: block; background: rgba(124, 58, 237, 0.2); border: 1px solid var(--accent2); color: var(--accent2); }
    .destination-status .countdown { font-size: 1.1rem; margin-top: 6px; }
  </style>
</head>
<body>
  <div class="app">
    <header class="header">
      <h1>EV Live — Sunnyvale → Death Valley</h1>
      <div class="route-label">Sunnyvale → Death Valley → Sunnyvale (round trip) · __TOTAL_KM__ km · Loop</div>
      <div class="controls">
    ...
    <aside class="sidebar">
      ...
      <div class="destination-status" id="destinationStatus" role="status">
        <span id="destinationStatusText"></span>
        <div class="countdown" id="destinationCountdown"></div>
      </div>
      ...
      <div class="soc-warning" id="socWarning" role="alert"></div>
      <div class="progress-block" id="chargingBlock" style="display: none;">
        <label id="chargingLabel">Charging at destination</label>
        <div class="progress-track">
  ...
  <script>
    ...
    var totalKm = typeof routeData.total_km === 'number' ? routeData.total_km : 542;
    var maxRangeKm = typeof routeData.max_range_km === 'number' ? routeData.max_range_km : 300;
    ...
    function windDirFromDeg(deg) {
      ...
      var i = Math.round(((deg % 300) + 300) % 300 / 45) % 8;
      return d[i];
    }
    ...
    function updateUI(data) {
      ...
      var socWarning = document.getElementById('socWarning');
      if (socWarning) {
        socWarning.className = 'soc-warning';
        socWarning.textContent = '';
        if (!data.charging && soc <= 20 && soc > 0) {
          socWarning.className = 'soc-warning low';
          socWarning.textContent = '⚠ Low battery (' + soc.toFixed(0) + '%) — find a charger';
        } else if (soc <= 0 && data.empty_recovery_seconds_left != null && data.empty_recovery_seconds_left > 0) {
          socWarning.className = 'soc-warning empty';
          var locName = (waypoints[data.current_index] && waypoints[data.current_index].label && waypoints[data.current_index].label.trim()) ? waypoints[data.current_index].label.trim() : 'current location';
          socWarning.textContent = 'Battery empty — navigation stopped. Charging at ' + locName + ' in ' + data.empty_recovery_seconds_left + ' s.';
        } else if (!data.charging && soc <= 0) {
          socWarning.className = 'soc-warning empty';
          socWarning.textContent = 'Battery empty — charging to continue trip';
        }
      }
      ...
      setText('navTotalKm', data.total_km != null ? Math.round(data.total_km) : totalKm);
      var chargingBlock = document.getElementById('chargingBlock');
      var chargingLabel = document.getElementById('chargingLabel');
      ...
      if (data.charging && chargingBlock && chargingFill && chargingText) {
        ...
        if (chargingLabel) chargingLabel.textContent = data.recovery_charging ? ('Charge at ' + (w.label && w.label.trim() ? w.label.trim() : 'current location')) : 'Charging at destination';
        chargingFill.style.width = pct.toFixed(0) + '%';
        chargingText.textContent = data.recovery_charging ? (pct.toFixed(0) + '% — Charge to 100%, CONTINUE TRIP') : (pct.toFixed(0) + '%');
      } else if (chargingBlock) {
        chargingBlock.style.display = 'none';
      }
      var destStatus = document.getElementById('destinationStatus');
      var destStatusText = document.getElementById('destinationStatusText');
      var destCountdown = document.getElementById('destinationCountdown');
      if (destStatus && destStatusText && destCountdown) {
        if (data.destination_reached && data.destination_countdown_sec != null) {
          destStatus.className = 'destination-status reached';
          destStatus.style.display = '';
          destStatusText.textContent = 'Destination reached';
          destCountdown.textContent = 'Start 10 sec count down to next destination — ' + data.destination_countdown_sec + ' s';
          destCountdown.style.display = 'block';
        } else if (!data.charging && data.trip_remaining_km != null && data.trip_remaining_km > 0 && data.trip_remaining_km < 100) {
          destStatus.className = 'destination-status approaching';
          destStatus.style.display = '';
          destStatusText.textContent = 'Destination approaching';
          destCountdown.style.display = 'none';
        } else {
          destStatus.className = 'destination-status';
          destStatus.style.display = 'none';
          destCountdown.style.display = 'none';
        }
      }
    ...
    function poll() {
      var url = (window.location.origin || 'http://127.0.0.1:5004') + '/api/state';
      fetch(url).then(function(r) { return r.json(); }).then(updateUI).catch(function() {});
    }

    document.getElementById('btnPlay').onclick = function() { fetch((window.location.origin || 'http://127.0.0.1:5004') + '/api/play').then(poll); };
    document.getElementById('btnPause').onclick = function() { fetch((window.location.origin || 'http://127.0.0.1:5004') + '/api/pause').then(poll); };
    document.getElementById('btnReset').onclick = function() { fetch((window.location.origin || 'http://127.0.0.1:5004') + '/api/reset').then(poll); };
    document.getElementById('speedSelect').onchange = function() {
      var v = this.value;
      fetch((window.location.origin || 'http://127.0.0.1:5004') + '/api/speed?x=' + v).then(poll);
    };
    ...
    if (initialState) {
      ...
      setText('navDestination', 'Run: python tests/ev_navigation_sunnyvale_deathvalley_ui.py --web');
    }
    poll();
...
def main() -> int:
    parser = argparse.ArgumentParser(description="EV Live Navigation: Sunnyvale → Death Valley National Park")
    parser.add_argument("--web", action="store_true", help="Run browser UI")
    parser.add_argument("--port", type=int, default=5004, help="Port for web UI")
    ...
    one_way_km = route_data["total_km"]
    route_data = _make_round_trip(route_data)
    route_data["one_way_km"] = one_way_km
    waypoints = route_data["waypoints"]
    n = len(waypoints)
    max_range_km = route_data.get("max_range_km", 300)
    ...
    step_sec = 1.0  # 1 waypoint every 2s at 1× (2/speed s at higher speeds)
    charge_step_pct = 1  # charging at destination: +1% per tick
    ...
    def simulation_loop() -> None:
        while True:
            ...
            with state_lock:
                ...
                if state["soc_pct"] <= 0:
                    state["playing"] = False
                    state["empty_recovery_at"] = time.time() + 10
